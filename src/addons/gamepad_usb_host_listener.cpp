...
#define DS_LIGHTBAR_SETUP_LIGHT_OUT    (1 << 1)
...
void GamepadUSBHostListener::process() {
    ...
    // 2. Forzar actualización de color (Esto anulará el azul del sistema)
...
// --------------------------------------------------------------------------------
//        ENVÍO AGRESIVO DE LED (ROJO / BLANCO)
// --------------------------------------------------------------------------------
...
void GamepadUSBHostListener::init_ds5_led(uint8_t dev_addr, uint8_t instance) {
    ...
    // Habilitamos TODO para que el mando nos haga caso a nosotros y no al sistema
    ...
    out_report.led_brightness = 0xFF; // Brillo máximo
    out_report.player_leds = 0x00;    // Apagar los puntitos blancos de abajo para que no estorben

    out_report.lightbar_red   = LED_EAFC_R; 
    out_report.lightbar_green = LED_EAFC_G;
    out_report.lightbar_blue  = LED_EAFC_B;

    // Forzar el envío hasta que entre
    int reintentos = 0;
...
// --------------------------------------------------------------------------------
//        PROCESAR INPUT Y CAMBIO DE PERFIL
// --------------------------------------------------------------------------------

void GamepadUSBHostListener::process_ds(uint8_t const* report, uint16_t len) {
    if (report[0] != 1) return; // Solo procesar reportes de botones

    DSReport controller_report;
    memcpy(&controller_report, report, sizeof(controller_report));

    // Lógica de cambio de perfil (Select + Start por 1 segundo)
    if (controller_report.buttonSelect && controller_report.buttonStart) {
        if (!profile_switch_held) {
            ...
            ds5_led_needs_update = true; // Activar actualización de LED
...
// Funciones requeridas vacías o básicas
